...
function startCountdown() {
    const countdown = setInterval(() => {
        ...
        if (distance < 0) {
            ...
            if (countdownContainer) countdownContainer.innerHTML = "<h3>┬íEl gran d├¡a ha llegado!</h3>";
            triggerCelebration();
            return;
        }
...
async function startHeroTyping() {
    const nameText = "Zo├® Roque D├¡az";
    const line2 = "Te invito a celebrar conmigo este d├¡a tan especial";
    ...
    audioBtn.addEventListener('click', (e) => {
        ...
        if (bgMusic.paused) {
            bgMusic.play().then(() => {
                ...
                audioBtn.setAttribute('aria-label', "Pausar m├║sica");
            }).catch(err => {
...
function copyToClipboard(text) {
    navigator.clipboard.writeText(text).then(() => {
        alert('┬íN├║mero de cuenta copiado!');
    }).catch(err => {
...
if (shareBtn) {
    shareBtn.addEventListener('click', async () => {
        const shareData = {
            title: 'Invitaci├│n a los XV A├▒os de Zo├® Roque D├¡az',
            text: '┬íAcomp├í├▒ame a celebrar mis XV a├▒os! Te espero con mucha ilusi├│n.',
...
if (calendarBtn && calendarOptions) {
    ...
    const eventDetails = {
        title: "XV A├▒os de Zo├® Roque D├¡az",
        description: "Te invito a celebrar mis XV a├▒os en este sue├▒o hecho realidad.",
        location: "Jardin ÔÇ£ManzanaresÔÇØ, Ignacio Allende 515, Casa Blanca, 62587 Temixco, Mor.",
...
function renderCloudinaryGallery(resources) {
    ...
    html += `
        <div class="photo-gallery-latest reveal active">
            <span class="photo-badge">├Ültima Foto</span>
            <img src="${latestUrl}" alt="├Ültima foto subida" onclick="openVisor('${latestUrl}')" style="cursor: pointer;">
        </div>
    `;
        ...
        try {
            ...
            if (navigator.canShare && navigator.canShare({ files: [file] })) {
                await navigator.share({
                    files: [file],
                    title: 'C├│digo QR de mis XV A├▒os',
                    text: 'Escanea este c├│digo para compartir tus fotos conmigo.',
            ...
            if (navigator.share) {
                await navigator.share({
                    title: '├ülbum de Fotos de los XV A├▒os',
                    text: '┬íSube tus fotos aqu├¡!',
